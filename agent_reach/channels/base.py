...
class Channel(ABC):
    ...
    description: str = ""             # e.g. "YouTube videosubtitles"
    ...
    def check(self, config=None) -> Tuple[str, str]:
        ...
        """
        self.active_backend = self.backends[0] if self.backends else ""
        return "ok", f"{','.join(self.backends) if self.backends else ''}"
